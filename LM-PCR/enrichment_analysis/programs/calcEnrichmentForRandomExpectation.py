...
def countOverlapsInObservedData(F1_name,F2_name,Chrom_list_f_name,Distance):
  import subprocess

  Cmd = 'bedtools slop -i %(f2_name)s -g %(chrom_list_f_name)s -b %(distance)d | \
       bedtools intersect -a "stdin" -b %(f1_name)s | cut -f 1-3 | sort | uniq | wc -l | cut -d " " -f 1' % {'f1_name':F1_name,'f2_name':F2_name,'chrom_list_f_name':Chrom_list_f_name,'distance':Distance}
  Count = subprocess.check_output(Cmd, shell=True).strip()
  Count = int(Count)
  return Count

#countOverlapsInRandomizedData()
def countOverlapsInRandomizedData(F1_name,F2_name,Chrom_list_f_name,Distance,NumRandomization):
  import subprocess
  import numpy as np

  Cmd = 'bedtools shuffle -i %(f2_name)s -g %(chrom_list_f_name)s -noOverlapping | \
         bedtools slop -i "stdin" -g %(chrom_list_f_name)s -b %(distance)d | \
         bedtools intersect -a "stdin" -b %(f1_name)s | cut -f 1-3 | sort | uniq | wc -l | cut -d " " -f 1' % {'f1_name':F1_name,'f2_name':F2_name,'chrom_list_f_name':Chrom_list_f_name,'distance':Distance}
...
for chip_name in chip_f_name_d:
  ...
  count = countOverlapsInObservedData(IS_f_name,chip_f_name,chrom_list_f_name,distance)
  mean,var,std = countOverlapsInRandomizedData(IS_f_name,chip_f_name,chrom_list_f_name,distance,num_of_randomaization)
  if mean > 0:
