...
geneset <- read.table(args[2], header=F, check.names=F)
exp <- readRDS(args[3])
geneset <- geneset[geneset$V1 %in% rownames(exp),]
T_l <- list(geneset)
names(T_l) <- "NFKB_regulated_genes"
Th <- gsva(as.matrix(log10(exp + 1)), T_l, mx.diff=TRUE, 
                 verbose=T,parallel.sz=4,min.sz=10,max.sz=500,method="ssgsea")

exp.fil <- exp[rownames(exp) %in% geneset,]
...
geneset <- geneset[geneset %in% names(cor.fil)]
T_l <- list(geneset)
names(T_l) <- "NFKB_regulated_genes"
Th <- gsva(as.matrix(log10(exp + 1)), T_l, mx.diff=TRUE, 
                 verbose=T,parallel.sz=4,min.sz=10,max.sz=500,method="ssgsea")

combined$NFKB_regulated_genes <- as.numeric(t(Th))
